...
**Important:** `composer install` can take 5-15 minutes. Use the **maximum timeout (600000ms)** for the Bash tool call. Always run with unlimited memory: `php -d memory_limit=-1 $(which composer) install`.
...
**Important:** `drush site:install` can take 10-30 minutes (especially with multilingual + recipes). Use the **maximum timeout (600000ms)** for the Bash tool call. Always run drush with unlimited memory: `php -d memory_limit=-1 vendor/bin/drush site:install ...`
